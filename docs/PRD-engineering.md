...
- R: **State v12**, `settings: { elapsedTickEnabled }`. Two fields have been dropped by migration rather than mapped, each because its only reader went: a pre-v11 snapshot's `phase` (a position inside a half of a session that no longer has halves), and v11's `settings.userLang` (which filtered a course dropdown that is now one list of pairs). Both steps cost a preference or a place, never progress — counters, review queue, passed modules and session count ride through untouched.
- R: **Settings is three cards** — Course, Practice, Backup. The course control is ONE select over every course, by pair (`hindi → marathi`); there is no separate "your language" step, because the pair is the whole choice. Storage is a single sentence from `navigator.storage.estimate()` inside Backup, never a meter or a per-course breakdown: there is no per-course delete, so a byte figure per course is a number nobody can act on. Import still shows both sides before it replaces anything (F8), which is the one place on the screen where more detail is right.
