...
- **Course / courseId** — one L1→L2 pair with its own content, strings, progress (`hi-mr`, `en-es`, `en-ar` and `hi-en` all ship; the `fixture: true` seam stays for a course #5 authored behind the gate).
