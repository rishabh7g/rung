 ...
 *
 * - **`glossEn` is OPTIONAL, and usually absent** (#405). The L1 cue is already English, so a gloss
 *   earns its place only when it carries something the `literal` does not — a register, a gender, a
 *   plural English lacks. The build requires it only where neither language of the pair is English.
 ...
 *
 * - **`glossEn` is OPTIONAL, and usually absent** (#405). The L1 cue is already English, so a gloss
 *   earns its place only when it says something the `literal` does not. `checkGlossEn` requires it
 *   only where neither language of the pair is English.
...
export const COURSE_BRIEFS: Readonly<Record<string, Readonly<Record<string, ModuleBrief>>>> = {
  ...
  'en-fr': {
    'L1-M1': {
      ...
      notes: [
        'LANGUAGE OF THE FIELDS, settled once for the course: the document speaks the course\'s L1, so every teaching field — rules[].text, word note, trap, sound, variations[].changed, mistake.why, usage, mnemonic and cue — is ENGLISH, and French appears only in the L2 slots: sentence / word / variation / mistake / pool display, and word forms. An English field may quote the French it explains; quoting is not switching. glossEn is OPTIONAL (#405): write one only where a natural English reading differs from the cue and from the literal; a gloss that repeats either is dropped. literal is the tool wherever the French construction is not word for word — je m\'appelle is "I call-myself", il y a is "it there has".',
  ...
  'en-de': {
    'L1-M1': {
      ...
      notes: [
        'LANGUAGE OF THE FIELDS, settled once for the course: the document speaks the course\'s L1, so every teaching field — rules[].text, word note, trap, sound, variations[].changed, mistake.why, usage, mnemonic and cue — is ENGLISH, and German appears only in the L2 slots: sentence / word / variation / mistake / pool display, and word forms. An English field may quote the German it explains; quoting is not switching. glossEn is OPTIONAL (#405): write one only where a natural English reading differs from the cue and from the literal; a gloss that repeats either is dropped. literal is the tool wherever the German construction is not word for word, and German needs it more often than French does: Ich habe Brot gegessen is "I have bread eaten", Wie geht es Ihnen? is "how goes it to-you", Es gibt einen Stuhl is "it gives a chair". Hyphenate a multi-word English gloss of one German word, as en-es hyphenates call-myself.',
