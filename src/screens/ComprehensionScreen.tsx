 ...
 *
 *   • **It IS the exit ritual now** (#348). The ritual used to open on an arc that asked the
 *     learner to write an eleventh sentence in a notebook and sign it with a ~900ms hold, and
 *     only a paid hold handed over to this screen. The product retired notebook writing, so the
 *     arc, the hold and its hand-over token went with it, and `/ritual` renders this screen
 *     directly. What guards it is what always did the real work: the COUNTERS. A rung that is not
 *     `exit_available` — passed a moment ago, or not yet marked through — has no test to sit, and
 *     lands on its module, which is where the work is.
...
function ComprehensionTest({ moduleId, open }: ComprehensionTestProps) {
  ...
  /**
   * And the counters, asked once they can answer: a rung whose sentences are not all marked has no
   * test to sit. It is the very predicate the Ladder's card reads (`exit_available`, #95), so the
   * two cannot disagree — and it is live, so passing this rung (#103) closes the screen behind the
   * learner rather than leaving a back tap pointing at a test for a rung already climbed.
   *
   * It sends them to the MODULE, which is where the work is — the arc used to make that call
   * before #348 folded it into this screen. It deliberately does not send them to `/ritual`: that
   * route is this screen, and a screen that redirected to itself would spin.
   */
  if (!open) return <Navigate to={`/module/${moduleId}`} replace />;
