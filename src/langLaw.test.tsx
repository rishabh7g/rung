...
describe('every course names its two languages in tags, not just in words', () => {
  ...
  it('is English about German for the eighth course (#365)', () => {
    ...
    // The row carries no fixture flag — #365 deleted it, so a learner build ships the course.
    expect(enDe?.l1Tag).toBe('en');
    expect(enDe?.l2Tag).toBe('de');
    expect(enDe?.fixture).toBeUndefined();
  });
