 ...
 *
 * Shape: the file is authored nested (`{"settings":{"tick":{"title":…}}}`) and read flat
 * (`strings['settings.tick.title']`), because the canonical list is dot-paths and the app should
...
/** Reads a dot-path out of the nested file: `settings.tick.title` → `{settings:{tick:…}}`. */
