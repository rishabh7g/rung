...
describe('loadCourses', () => {
  it('reads the courses out of the emitted envelope, extra course keys intact', async () => {
    ...
    // en-es graduated in #195, en-ar in #202, hi-en in #273 and en-it in #337 — a shipping course
    ...
    expect(courses[4]).toMatchObject({ id: 'en-it', l1Tag: 'en', l2Tag: 'it' });
  });

  /**
   * `--with-fixtures` and the `fixture` key are the seam a course is authored behind (PRD §17):
   * hi-en was that course from #267 until #273 graduated it, and en-it from #332 until #337. The
   * loader keeps the key rather than validating it away, so the row reaches the app intact — the
   * key is what `resolveActiveCourse` and the Settings switcher never branch on, and what
   * graduation deletes. No authored row carries it today, so the test flags one of its own.
   */
