...
// The five product faces, self-hosted [D15] and right-sized (#113, #197, #222). The prototype pulls Mukta
...
// draws Arabic. Source Sans 3 is the same shape again (#222), cut to the four diacritics of the
// romanization Mukta has no glyph for. `vite.config.ts` drops @fontsource's `.woff` fallbacks so
...
if (import.meta.env.DEV) {
  ...
  // docs/04-font-notes.md §4). Still dev-only after #222, and for a sharper reason than before:
  // Barlow never draws the romanization — every L2 line is `--font-devanagari`, which is now
  // Mukta then Source Sans 3 — and Barlow's latin-ext has no ḥ, ʾ or ʿ to contribute anyway.
  // Dynamic imports inside a DEV branch never enter a production graph — the
  // `src/dev/typeRoute.tsx` pattern.
