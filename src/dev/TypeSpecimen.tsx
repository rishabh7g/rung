...
const SIZES = [
  { px: 18, className: 'specimen-s18', token: '--devanagari-min-size (body-role floor)' },
  { px: 22, className: 'specimen-s22', token: '--text-l2-list' },
  { px: 26, className: 'specimen-s26', token: '--text-l2-card' },
  { px: 32, className: 'specimen-s32', token: '--text-l2-hero' },
] as const;
...
/** The ramp's Mukta weights — 400 (cue), 600 (card/list), 700 (hero). 500 was headroom no token
    ...
    synthesised face. */
const WEIGHTS = [
  { weight: 400, className: 'specimen-w400' },
  { weight: 600, className: 'specimen-w600' },
  { weight: 700, className: 'specimen-w700' },
] as const;
...
const BODY_STEPS = [
  { className: 'specimen-body', token: '--text-body (15px)' },
  { className: 'specimen-secondary', token: '--text-secondary (13px)' },
] as const;
...
const BODY_WEIGHTS = [{ weight: 400, className: 'specimen-w400' }] as const;

/** Barlow Condensed: the kickers, their tracking, and the wordmark's own 700. */
const HEADING_STEPS = [
  { className: 'specimen-kicker-lg', token: '--text-kicker (600 11px) + --kicker-tracking' },
  { className: 'specimen-kicker-sm', token: '--text-kicker-sm (600 10px) + --kicker-tracking' },
  { className: 'specimen-brand', token: '--text-brand (700 23px)' },
  { className: 'specimen-screen-title', token: '--text-screen-title (600 24px)' },
] as const;

export default function TypeSpecimen() {
  return (
    <div className="specimen-page">
      <h1 className="specimen-title">type specimen</h1>
      ...
      <section className="specimen-section">
        <h2 className="specimen-section-head">Devanagari · Mukta · --font-devanagari</h2>
        {SIZES.map((size) => (
          <div key={size.px} className="specimen-block" data-size={size.px}>
            <h3 className="specimen-block-head">
              {size.px}px — {size.token}
            </h3>
            {WEIGHTS.map((weight) => (
              <div key={weight.weight} className="specimen-row">
                <span className="specimen-row-label">{weight.weight}</span>
                <ul
                  className={`specimen specimen-devanagari ${size.className} ${weight.className}`}
                  lang="mr"
      ...
      <section className="specimen-section">
        <h2 className="specimen-section-head">
      ...
      <section className="specimen-section">
        <h2 className="specimen-section-head">Romanization · Barlow · --font-body</h2>
        {BODY_STEPS.map((step) => (
          <div key={step.token} className="specimen-block">
            <h3 className="specimen-block-head">{step.token}</h3>
            {BODY_WEIGHTS.map((weight) => (
              <div key={weight.weight} className="specimen-row">
                <span className="specimen-row-label">{weight.weight}</span>
                <ul
                  className={`specimen ${step.className} ${weight.className}`}
                  data-face="body"
                  data-weight={weight.weight}
                >
                  {ROMANIZATION.map((sample) => (
      ...
      <section className="specimen-section">
        ...
        {HEADING_STEPS.map((step) => (
          <div key={step.token} className="specimen-block">
            <h3 className="specimen-block-head">{step.token}</h3>
            <p className={`specimen ${step.className}`} data-face="heading">
